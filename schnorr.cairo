...
# returns 1 for verified, 0 for error
func verify{range_check_ptr}(public_key: EcPoint, signature : Signature, message: felt) -> (res: felt):
    alloc_locals
    ...
    let (local g_to_the_s: EcPoint) = ec_mul(gen_pt, signature.s)
    ...
    if signature.e.d0 == e_v.d0:
        if signature.e.d1 == e_v.d1:
            if signature.e.d2 == e_v.d2:
                return (res = 1)
            end
        end
    end
